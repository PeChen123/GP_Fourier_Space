...
class FTMAGI(object):
    def __init__(self, ys, dynamic, grid_size=201, interpolation_orders=3):
        ...
        self.comp_size = len(ys) #observarion 
        for i in range(self.comp_size):
            if (not torch.is_tensor(ys[i])): # if not tensor, convert to tensor
    ...
    def map(self, max_epoch=1000, 
            ...
            verbose=False, returnX=False,truncated=False, k = None):
        ...
        with torch.no_grad():
            for i in range(self.comp_size):
                ...
                if truncated and k is not None:
                    LKinv_freq = torch.fft.fft(LKinv_tensor)[:k, :k]
                    # Normalization after performing the FFT 
                    LKinv_freq /= len(LKinv_tensor)
                else:
                    LKinv_freq = torch.fft.fft(LKinv_tensor)
                    # Normalization after performing the FFT 
                    LKinv_freq /= len(LKinv_tensor)
                ...
                x[:,i] = xi
                dxdtGP[:,i] = dxi
                gpmat.append({'LC':LC,'LCinv':LCinv,'m':m,'LKinv':LKinv,'s':s,'LQinv':LQinv,'LU':LU, 'LKinv_freq': LKinv_freq})
            ...
            for st in range(1):
                ...
                for i in range(self.comp_size):
                    ...
                    if truncated and k is not None:
                        ...
                        dxdtGP_freq = torch.fft.fft(gpmat[i]['m'].matmul(x[:, i] - mean))[:k]
                        dxdtGP_freq /= len(gpmat[i]['m'].matmul(x[:, i] - mean))  # Normalize
                        
                        diff_freq = dxdtOde_freq - dxdtGP_freq
                        dxidtError_fft = gpmat[i]['LKinv_freq'].matmul(diff_freq)  
                    else:
                        dxdtOde_freq = torch.fft.fft(dxdtOde[:, i]).real
                        dxdtOde_freq /= len(dxdtOde[:, i])  # Normalize
                        
                        dxdtGP_freq = torch.fft.fft(gpmat[i]['m'].matmul(x[:, i] - mean))
                        dxdtGP_freq /= len(gpmat[i]['m'].matmul(x[:, i] - mean))  # Normalize
