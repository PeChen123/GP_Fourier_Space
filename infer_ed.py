...
class FMAGI(object):
    ...
    def map(self, max_epoch=1000, 
        ...
        # replaing with z 
        z = torch.empty(self.grid_size, self.comp_size).double()
        ...
        with torch.no_grad():
            for i in range(self.comp_size):
                ...
                eval, evec = torch.linalg.eigh(C_d)
                eval_sqrt = torch.diag(np.sqrt(eval)) # 161x161
                K = evec.matmul(eval_sqrt)
                Kinv = K.inverse()
                ...
                z[:,i] = zi
                dxdtGP[:,i] = dxi
                gpmat.append({'LC':LC,'LCinv':LCinv,'m':m,'LKinv':LKinv,'s':s,'LQinv':LQinv,'LU':LU,'K':K,'Kinv':Kinv})
        ...
        for epoch in range(max_epoch):
            ...
            # nor, we optimize z (x after eigen decomposition)
            z.requires_grad_(True)
            for st in range(1):
                state_optimizer.zero_grad()
                # reconstruct x
                x = torch.empty_like(z).double()
                for i in range(self.comp_size):
                    mean = self.gp_models[i].mean_module.constant.item()
                    outputscale = self.gp_models[i].covar_module.outputscale.item()
                ...
                for i in range(self.comp_size):
                    ...
                    # extend x to be more densor in 201
