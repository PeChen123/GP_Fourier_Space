    ...
    def map(self, max_epoch=1000, 
        ...
        with torch.no_grad():
            for i in range(self.comp_size):
                ...
                s = s.t().matmul(LCinv)
                # compute covariance for inter|grid
                m_int = LCinv.matmul(base_kernel.dCdx2(self.interp,self.grid)).t()
                LKinv_int = (base_kernel.d2Cdx1dx2(self.interp,self.interp) - m_int.matmul(m_int.t())).add_jitter(1e-6)._cholesky().inverse()
        ...
        for tt in range(200):
            ...
            for i in range(self.comp_size):
                mean = self.gp_models[i].mean_module.constant.item()
                outputscale = self.gp_models[i].covar_module.outputscale.item()
                dxrdtError = gpmat[i]['LKinv_int'].matmul(dxrdtOde[:,i]-gpmat[i]['m_int'].matmul(xr[:,i]-mean))
                lkh[i] = -0.5/outputscale * dxrdtError.square().mean()
            theta_loss = -torch.sum(lkh)
            theta_loss.backward()
        ...
        for epoch in range(max_epoch):
            ...
            for st in range(1):
                ...
                # print('ode',dxdtOde[1:5,:])
                # print('ode_int',dxdtOde_int[1:5,:])

                # print('interp',x[1:10,:])
                # print('interp',x_i[1:10,:])
                ...
                for i in range(self.comp_size):
                    ...
                    if (interpolation):
                        dxidtError = gpmat[i]['LKinv_int'].matmul(dxdtOde_int[:,i]-gpmat[i]['m_int'].matmul(x[:,i]-mean))
